...
awk -F ',' '{print "    <channel lang=\"sk\" xmltv_id=\""$1"\""" site_id=\""$2"\""" logo=\""$5"\""">"$3"</channel>"}' solocoo.tv_sk.chanell.csv > solocoo.tv_sk.chanell.xml
sed -i 1i"<?xml version=\"1.0\" encoding=\"UTF-8\"?>" solocoo.tv_sk.chanell.xml
sed -i 2i"<site site=\"solocoo.tv_sk\">" solocoo.tv_sk.chanell.xml
sed -i 3i"\ \ <channels>" solocoo.tv_sk.chanell.xml
echo "  </channels>" >> solocoo.tv_sk.chanell.xml
echo "</site>" >> solocoo.tv_sk.chanell.xml
