...
NR>2 {
    for (i=1; i<=NF; i++) {
        printf("prefix " "#" ${{ secrets.SCRAPER }} "https://www.teraz.sk/historicky-kalendar/" $i "-" mm "&selector=h3,p&scrape=attr&spaced=true&attr=&pretty=true#" " | jq >> " mm "/" $i"-"mm"-1test.json" "\n")
