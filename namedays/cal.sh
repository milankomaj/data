...
NR>2 {
    for (i=1; i<=NF; i++) {
        printf("prefix " "#https://" System.getenv("SCRAPER") "/?url=https://www.teraz.sk/historicky-kalendar/" $i "-" mm "&selector=h3,p&scrape=attr&spaced=true&attr=&pretty=true#" " | jq >> " mm "/" $i"-"mm"-1test.json" "\n")
